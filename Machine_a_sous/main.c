...
KeyEvent Game(random_wheel* rw){
    ...
    while (rw->money>0){ //tant que l'utilisateur a de l'argent
        ...
        if (t!=NULL){ //en cas de gain (si la combinaison est trouvé dans l'arbre)
            //t->value=1000; //pour déclencher le jackpot à chaque gain enlever cette ligne des commentaires
            if (t->value==1000){ //jackpot
                system("play -q data/jackpot.wav 2>/dev/null &");
                color c1,c2;
                int j=(bet==3)?5:bet;
                for (int i=0;i<=j*500;i+=j*5){
                    usleep(40*1000);
                    c1=(color){100,(5/j)*(i%(j*60)),100}; //animation qui fait varier les couleurs
                    c2=(color){255,255,(5/j)*(i%(j*60))};
                    SET_COLOR(c1,c2);
